 ...
 */ 
    ...
    /**
     * Ensures that this bidder exists in MySQL after an admin full-clear or manual table edits.
     * Manual DB cleanup can leave a live Java Bidder object with an id that no longer exists.
     * This method recreates the row and refreshes the id before new auctions are inserted.
     */
    public synchronized boolean ensureDatabaseRow()
    {
        String name = getName();
        ...
        try
        {
            ResultSet set = db.query("SELECT `id` FROM `bidder` WHERE `name`=? LIMIT 1", name);
            try
            {
                if (set.next())
                {
        ...
        try
        {
            int type = "*Server".equalsIgnoreCase(name) ? 1 : 0;
            db.exec("INSERT INTO `bidder` (`name`, `type`, `notify`) VALUES (?, ?, ?)", name, type, 0);
            ResultSet set = db.query("SELECT `id` FROM `bidder` WHERE `name`=? LIMIT 1", name);
            try
            {
                if (set.next())
        ...
        {
            AuctionHouse.getInstance().getLogger().severe("AuctionHousAqua: failed to recreate bidder row for " + name + ": " + ex.getMessage());
        }
        return this.id > 0;
    }
